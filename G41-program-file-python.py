...
class Graph:
    ...
    def cycle_det(self, start_vertex, visited, lc):
        label_count = lc
        ...
        for i in range(self.__n):
            ...
            if Graph.__g[start_vertex][i] == 1 and (visited[i] == 0):
                ...
                self.cycle_det(i, visited, label_count)
            if Graph.__g[start_vertex][i] == 1 and (visited[i] != -1):  # check this != condition over here
                print("\nCycle detected :")
                self.hascycle = True
...
while True:
    ...
    if val == 1:
        ...
        print("Choose one of the option below: ")
        print("1) Check if there is a cycle in the graph or not")
        print("2) Check and randomly add edges to the graph until a cycle is formed")
        ...
        if (option == 1):
            ...
            lc = 1
            for xii in range(5):
                obj.cycle_det(xii, visited, lc)
            if (obj.hascycle == False):
                print (">> NO, there's no cycle in the graph")
            else:
                print("\n>> YES, there's cycle in the graph, printed out above")
            ...
            if (obj.hascycle == False):
                ...
                for xiii in range(5):
                    while (obj.hascycle == False):
                        obj.random_edge()
                        obj.cycle_det(xiii, visited, lc)
            else:
                print(">> The graph already has cycle in it and it is printed out below !!")
                visited = [0] * v
                lc = 1
                for xii in range(5):
                    obj.cycle_det(xii, visited, lc)
